"""
NSE Dual Trend Dual Timeframe (DTDTF) Swing Trader — DO Functions Entry Point
==============================================================================
Uses only requests + standard library (no pip installs needed).
- Yahoo Finance API for price data (daily + weekly)
...
def compute_dual_trend(bars, lookback):
    """
...
def send_email(exits, entries, holds):
    ...
    subject   = f"NSE DTDTF — {today} | {len(entries)} new | {len(holds)} open"

    def table_style():
        return 'border-collapse:collapse;width:100%;font-family:Arial,sans-serif;font-size:14px;'

    def th_style():
        return 'background:#1a3c5e;color:#fff;padding:8px 12px;text-align:left;'
    ...
    <h2 style="background:#1a3c5e;color:#fff;padding:14px 18px;margin:0;border-radius:4px 4px 0 0;">
        📊 NSE DTDTF Swing Trader — {today}
    </h2>
    <p style="font-size:12px;color:#888;margin:6px 0 0 2px;">
        Dual Trend · Daily (15-bar) + Weekly (12-bar) confluence
    </p>
    '''
...
def main(args):
    print("\n" + "="*55)
    print("  NSE DTDTF TRADER — DO Functions Run")
    print("="*55)
